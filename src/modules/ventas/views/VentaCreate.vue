  ...
  <Card v-if="venta">
    ...
    <CardBody>
      <div class="grid gap-6 mb-6 md:grid-cols-3">
        <div>
          <label for="client" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Cliente</label>
          <div class="flex">
            <v-select v-model="venta.client" id="client"
              ...
              :options="clients" label="label"></v-select>
            <button type="button"
          ...
          <NewClientModal :showModal="showNewClientModal" @on:close="toggleModal" />
          <NewReciboModal :showModal="showNewReciboModal" @on:close="toggleModal" />
          <p class="mt-2 text-sm text-red-600 dark:text-red-500" v-if="v$.client.$error">
        ...
        <div>
          ...
          <input type="date" v-model="venta.date" id="date"
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            placeholder="Fecha" required />
          <p class="mt-2 text-sm text-red-600 dark:text-red-500" v-if="v$.date.$error">
...
" disabled placeholder="Saldo" />
      ...
      <div class="grid gap-6 mb-6 md:grid-cols-4">
        ...
        <div>
          ...
          <div class="flex">
            ...
            <input
              ...
              type="number" v-model="venta.discountProduct" placeholder="0" required />
          </div>
        </div>
        <div>
          <label for="subtotalProduct"
            class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Subtotal</label>
          <input type="number" v-model="venta.subtotalProduct" id="subtotalProduct"
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        ...
        <div class="flex items-end justify-center">
          <button v-if="isEdit"
            ...
            type="button" :disabled="!venta.quantityProduct && !venta.discountProduct" @click="updateProduct">
            Actualizar
          </button>
          <button v-else
            class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"
            type="button" :disabled="!venta.quantityProduct && !venta.discountProduct" @click="addProduct">
            Agregar
          </button>
        </div>
      </div>
      ...
      <template v-if="venta.products.length > 0">
        <hr class="my-3" />
        <ProductsTable :products="venta.products" @on:edit="editProduct" @on:delete="deleteProduct" />
        <hr class="my-3" />
      </template>
      ...
      <div class="grid gap-6 mb-6 md:grid-cols-3">
        <div>
          <label for="subtotal" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Subtotal</label>
          <input type="number" :value="venta.subtotal" id="subtotal"
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            disabled />
          <p class="text-sm text-red-600 dark:text-red-500" v-if="v$.subtotal.$error">
            <span class="font-medium">El subtotal no puede ser menor o igual a 0</span>
          </p>
        </div>
        <div>
          ...
          <div class="flex">
            ...
            <input @change="calcDiscount"
              class="rounded-none rounded-r-lg bg-gray-50 border text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              type="number" v-model="venta.discountValue" placeholder="0" />
          </div>
        </div>
        <div>
          <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Total</label>
          <input type="number" :value="venta.total" disabled
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
          <p class="text-sm text-red-600 dark:text-red-500" v-if="v$.total.$error">
            <span class="font-medium">El total no puede ser menor o igual a 0</span>
          </p>
        </div>
      </div>

      <!-- Metodos de pago -->
      <label for="subtotal" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Metodo de Pago</label>
      <div :class="`grid gap-6 mb-6 grid-cols-5`">
        <div class="flex items-center pl-4 rounded border border-gray-200 dark:border-gray-700"
          v-for="paymentMethod in paymentMethods" :key="paymentMethod.id">
          <input :id="paymentMethod.value" type="radio" :value="paymentMethod.value" v-model="venta.paymentMethod"
            name="bordered-radio"
            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
          <label :for="paymentMethod.value"
            class="py-4 ml-2 w-full text-sm font-medium text-gray-900 dark:text-gray-300">{{ paymentMethod.label
            }}</label>
        </div>
        <p class="text-sm text-red-600 dark:text-red-500" v-if="v$.paymentMethod.$error">
      ...
      <div class="py-2">
        ...
        <Button className="danger" text="Cancelar" to="ventas-list" :loading="isLoading" />
      </div>
    </CardBody>
  </Card>
</template>
...
export default {
  ...
  setup() {
    ...
    const { showNewClientModal, showNewReciboModal, toggleModal } = useUI()
