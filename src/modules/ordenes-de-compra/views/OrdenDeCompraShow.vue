<template>
  <Card v-if="ordenDeCompra">
    <CardHeader>
      <TitleText
        :text="`Factura ${ordenDeCompra.factura} - ${
          ordenDeCompra.proveedor.name
        } - ${$filters.formatDate(ordenDeCompra.date)}`"
      />
    </CardHeader>
    <CardBody>
        ...
        <div>
          ...
          <div class="flex">
            ...
            <input
              class="rounded-none rounded-r-lg bg-gray-50 border border-gray-300 text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              type="number"
              :value="ordenDeCompra.total"
              readonly
            />
          </div>
        </div>
      </div>
      <ProductsTable :products="ordenDeCompra.products" :hiddenOptions="true" />
    </CardBody>
  </Card>
</template>
...
export default {
  ...
  setup() {
    ...
    const { loadOrdenDeCompra, ordenDeCompra } = useOrdenesDeCompra();

    loadOrdenDeCompra(id);
